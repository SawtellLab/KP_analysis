...
line(xtime,median(thisexpt.wc.Vm,1),'color','r','LineWidth',3)
axis tight

Vm_Istep = thisexpt.wc.Vm;
...
figure;hold on
for i = 1:size(Vm_ObjectMoved,1)
    
    scaley = 1;
    offset = (thisexpt.sweeps.position(i))*scaley;
%    line(xtime(plotwin)+offset,Vm_ObjectMoved(i,plotwin)- median(Vm_ObjectMoved(i,baselinewin),2),'color','k')
   line(xtime(plotwin)+offset,Vm_ObjectMoved(i,plotwin),'color','k')

end
axis tight
set(gca,'TickDir','out')
title(['Baseline Vm ' num2str(round(mean(median(Vm_ObjectMoved(:,baselinewin),2))))]);

onsetind = [];
peakval = [];
for i = 1:size(Vm_ObjectMoved,1)
    thisdata = Vm_ObjectMoved(i,plotwin) - median(Vm_ObjectMoved(i,baselinewin),2);
  onsetind(i)= min(find(thisdata>1));
  peakval(i) = max(thisdata);
end

figure;
scatter(thisexpt.sweeps.position,((artifactwin(end)+onsetind)*thisexpt.meta.dt*1000)-(stimonset_t*1000))
ylabel('onset')

figure;
scatter(thisexpt.sweeps.position,peakval)
ylabel('peakval')
